...
\begin{algorithm}
\caption{Instagram-Bot}\label{Insta}
\begin{algorithmic}[1]
\State{Bot} = InstaBot(login=``username",password="password",)
\While{true}{
\State{time.sleep(10 * 60)}%10minuten warten
\State{all
=
Get(https://api.instagram.com/v1/tags/search?q=SocialComputing\&access\_token=ACCESS-TOKEN})}
